"""
This script is taken from https://gist.github.com/mmozeiko/7f3162ec2988e81e56d5c4e22cde9977k

Downloads the compiler and linker for MSVC without the need for visual studio code bullshit.
"""
    ...
    for payload in dbg["payloads"]:
        ...
        with open(d / name, "wb") as f:
            data = download_progress(
                payload["url"], payload["sha256"], f"{pkg}/{name}", f
            )
            total_download += len(data)
    msi = (
        d / first(dbg["payloads"], lambda p: p["fileName"].endswith(".msi"))["fileName"]
    )

    with tempfile.TemporaryDirectory() as d2:
        subprocess.check_call(
            ["msiexec.exe", "/a", str(msi), "/quiet", "/qn", f"TARGETDIR={d2}"]
        )
...
with tempfile.TemporaryDirectory() as d:
    ...
    for payload in dia["payloads"]:
        ...
        with open(d / name, "wb") as f:
            ...
            total_download += len(data)
    msi = (
        d / first(dia["payloads"], lambda p: p["fileName"].endswith(".msi"))["fileName"]
    )
...
for arch in ["x86", "x64", "arm", "arm64"]:
    ...
    (OUTPUT / "VC/Tools/MSVC" / msvcv / f"bin/Host{arch}/{arch}/vctip.exe").unlink(
        missing_ok=True
    )
